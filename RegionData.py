...
class Region():
    def __init__(self, chrom, pos, start, end, ref, alt):
        self.chrom = chrom
        self.pos = pos
        self.start = start
        self.end = end
    ...
    def write(self):
        ...
        return '\t'.join([self.chrom, str(self.pos), str(
            self.pos), self.ref, self.alt, FlatTensor]) + '\n'
...
def CreateRegion(RefFile, SamFile, BamoutFile, chrom, pos, ref, alt, verbose=False):
    pos = int(pos)
    start = pos - (WIDTH - 1) / 2
    end = pos + (WIDTH - 1) / 2 + 1
    # print 'chr'+str(chrom)+':'+str(pos),start,end,start-end
    region = Region(chrom, pos, start, end, ref, alt)
...
def Line2Window(l):
    Chr, Start, End, Ref, Alt, Data = l.strip().split('\t')
    print 'Chrom: %s\tPos: %s\t%s->%s' % (Chr, End, Ref, Alt)
    start = 0
    height = 0
    depth = 0
    while depth < 3:
