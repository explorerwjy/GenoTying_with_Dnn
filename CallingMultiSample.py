...
class Reader:
    def __init__(self):
        self.VCFname = FLAGS.VcfName
        self.BamList = FLAGS.BamList
        self.ReaderThread = FLAGS.ReaderThread
        self.BamSampleMapperFil = FLAGS.BamSampleMapperFil
        self.VarOnly = FLAGS.VarOnly


    def GetVCF(self):
        if self.VCFname.endswith(".vcf.gz"):
            return gzip.open(self.VCFname)
        elif self.VCFname.endswithh(".vcf"):
            return open(self.VCFname)
        else:
            print "Error! --VCFname should be a vcf file."

    def GetBams(self):
        bams = open(self.BamList).readlines()
        if self.BamSampleMapperFil == None:
            self.SM2BAM = {}
            for bam in bams:
                tmp = pysam.AlignmentFil(bam, 'rb')
                SM = GetSM(tmp.header())
                if SM not in self.SMs:
                    print "%s not in VCF. Please check Samples in VCF are same as in BAMs"
                    exit()
                self.SM2BAM[SM] = tmp
    ...
    def run(self):
        ...
        for l in self.VCFhand:
            ...
            if l.startswith("##"):
                ...
                llist = l.split("\t")
                FMT, GTs = llist[9], llist[10:]
                #if self.VarOnly:
                for i,GT in enumerate(GTs):
                    GT = Genotype(FMT, GT)
                    if GT.isvar() or !(self.VarOnly):
                        Jobs.put(self.SM2BAM[self.SMs[i]], CHR, POS)
