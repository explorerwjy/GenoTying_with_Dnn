...
def GetOptions():
    ...
    parser.add_argument('-p', '--positive',
                        help='VCF file contains Positive variants')
    parser.add_argument('-c', '--candidate',
                        help='VCF file contains Candidate variants')
    parser.add_argument('-m', '--mode', type=str, choices=['train', 'test', 'all'],
                        help='mode for run. train will load chr1-19 from positive vcf. test will load chr20-22 from positive vcf. all will load all variants from positive vcf')
...
class Counts():
    ...
    def Get_POS_Eval(self):
        self.All = self.zero_zero+self.one_zero+self.two_zero+self.zero_one+self.zero_two+self.one_one+self.two_two+self.one_two+self.two_one
...
def Classify_2(l, True_dict, counts, TP_hand, FP_hand, FN_hand):
    k, v = var2kv(l)
    if k in True_dict:
        if v[0] == True_dict[k][0] and v[1] == True_dict[k][1] and True_dict[k][0] == True_dict[k][0] :  # Homozygous Alt
            counts.two_two += 1
            TP_hand.write(l)
...
def Classify(l, True_dict, counts, TP_hand, FP_hand, FN_hand):
    k, v = var2kv(l)
    if k in True_dict:
        if v[0] == v[0] and True_dict[k][0] == True_dict[k][0] :  # Homozygous Alt
            counts.two_two += 1
            TP_hand.write(l)
        elif v[0] != v[1] and True_dict[k][0] != True_dict[k][1] :  # Hetrozygous
            counts.one_one += 1
            TP_hand.write(l)
        elif v[0] != v[1] and True_dict[k][0] == True_dict[k][1] :  # 0/1 - 1/1
            counts.one_two += 1
            FP_hand.write(l)
        elif v[0] == v[1] and True_dict[k][0] != True_dict[k][1]:  # 1/1 - 0/1
            counts.two_one += 1
            FP_hand.write(l)
        else:
            print v,True_dict[k]
...
def GetTruthDict_All(PositiveVCF):
    ...
    print len(res)
    print "Read Truth Used %.3f s"%(time.time()-s_time)
    return res
